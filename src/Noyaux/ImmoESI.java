...
public class ImmoESI {
    public TreeSet<Bien> liste_des_biens = new TreeSet<Bien>(); //liste des biens de l'agence
    public TreeSet<Bien> biens_archives = new TreeSet<Bien>(); //liste des biens archivés
    public LinkedList<Messages> liste_messages = new LinkedList<Messages>();

    public TreeSet<Bien> getListe_des_biens() {
        return liste_des_biens;
    }

    public LinkedList<Administrateur> administrateurs = new LinkedList<Administrateur>();

    public void setProprietaires(TreeSet<Proprietaire> proprietaires) {
        this.proprietaires = proprietaires;
    }

    public TreeSet<Proprietaire> proprietaires = new TreeSet<Proprietaire>(); //liste de proprietaires
    ...
    public Bien AjouterMaison(Proprietaire proprietaire, TypeDeTransaction transaction) throws SuperficieHabitableException, SurfaceNulleException {
        ...
        System.out.println("Descriptif du bien ");
        String descriptif = sc.nextLine(); //sc.nextLine();
        System.out.println("Photos :(Entrez le lien URL vers l'album photos");
        String photo = sc.nextLine(); //sc.nextLine();
        Date date = new Date();
        System.out.println("Nombre d'etages : ");
        int nbr_etages = sc.nextInt();
        sc.nextLine();
        System.out.println("Nombre de pieces : ");
        int nbr_pieces = sc.nextInt();
        sc.nextLine();
        System.out.println("Cette maison contient-elle meublee ? ");
        String jar = sc.nextLine(); //sc.nextLine();
        boolean meuble;
        meuble = jar.equalsIgnoreCase("oui"); //sc.nextLine();
        System.out.println("Cette maison contient-elle un jardin ? ");
        jar = sc.nextLine(); //sc.nextLine();
        boolean jardin;
        jardin = jar.equalsIgnoreCase("oui"); //sc.nextLine();
        System.out.println("Cette maison contient-elle une piscine ? ");
        String pis = sc.nextLine();
        // sc.nextLine();
        boolean piscine;
        piscine = jar.equalsIgnoreCase("oui");
        System.out.println("Cette maison contient-elle un garage ? ");
        String gar = sc.nextLine(); //sc.nextLine();
        boolean garage;
        garage = jar.equalsIgnoreCase("oui");
        System.out.println("Surface habitable ");
        float surface_habitable = sc.nextFloat();
        sc.nextLine();
        //Creation de l'objet
    ...
    public Bien AjouterAppart(Proprietaire proprietaire, TypeDeTransaction transaction) throws SurfaceNulleException {
        ...
        System.out.println("A quelle étage se situe le bien? ");
        int etage = sc.nextInt();
        sc.nextLine();
        System.out.println("Ce bien est-il un duplexe ? ");
    ...
    public Bien AjouterTerrain(Proprietaire proprietaire, TypeDeTransaction transaction) throws SurfaceNulleException {
        ...
        String photo = sc.nextLine();
        Date date = new Date(); //Date d'ajout
        System.out.println("Nombre de façades du bien : ");
        int nbr_facades = sc.nextInt();
        sc.nextLine();
        System.out.println("Etat juridique : ");
        String etat_juridique = sc.nextLine();
    ...
    public void ajouterBien(Administrateur admin) throws SuperficieHabitableException, SurfaceNulleException, BienExistantException {
        ...
        sc.nextLine();
        System.out.println("Adresse du proprietaire ");
        String adresse_prop = sc.nextLine();
        System.out.println("Nom du proprietaire ");
        String nom = sc.nextLine();
        System.out.println("Prenom du proprietaire: ");
    ...
    public ArrayList<Bien> rech_filltre(ArrayList<Bien> list1, ArrayList<Bien> list2, int k) {
        ...
        if (k == 0) {
            ...
            if (choix == 1) {
                for (Bien ele1 : list1) {
                    if (ele1.getTransaction() == TypeDeTransaction.vente) {
                        list2.add(ele1);
                    }
                }
            } else if (choix == 2) {
                for (Bien ele1 : list1) {
                    if (ele1.getTransaction() == TypeDeTransaction.location) {
                        list2.add(ele1);
                    }
                }
            } else if (choix == 3) {
                for (Bien ele1 : list1) {
                    if (ele1.getTransaction() == TypeDeTransaction.echange) {
                        list2.add(ele1);
                    }
                }
            }
            ...
            if (choix == 2) {
                for (Bien ele1 : list1) {
                    if (ele1 instanceof Appartement) {
                        list2.add(ele1);
                    }
                }
            }
